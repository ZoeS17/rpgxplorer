...
<b>AC </b> <xsl:value-of select="AC"/>, <b>touch</b><xsl:text> </xsl:text><xsl:value-of select="ACTouch"/>, <b>flat-footed</b><xsl:text> </xsl:text><xsl:value-of select="ACFlatfooted"/>, <b>helpless</b><xsl:text> </xsl:text><xsl:value-of select="ACHelpless"/>
...
<xsl:if test="DR != ''"><b>DR </b><xsl:value-of select="DR"/><xsl:choose><xsl:when test="Feats/Feat[Tags/text()[contains(., 'Immunity')]] != '' or Features/Feature[Tags/text()[contains(., 'Immunity')]] != ''">; </xsl:when><xsl:otherwise><br/></xsl:otherwise></xsl:choose></xsl:if>
...
<xsl:for-each select="Spells/ClassSpells[not(ClassName = preceding-sibling::ClassSpells/ClassName)]">
	...
	<b><xsl:value-of select="ClassName"/> Spells
	<xsl:choose>
	...
	<xsl:for-each select="../../SpellCasterInfo/CasterClass">
		<xsl:if test="ClassName = $class">
			...
			<xsl:choose>
				...
				<xsl:otherwise>th; <xsl:value-of select="/CharacterXMLDataset/Character/Melee"/> melee touch, <xsl:value-of select="/CharacterXMLDataset/Character/Ranged"/> ranged touch):</xsl:otherwise>
			</xsl:choose>
		</xsl:if>
	</xsl:for-each>
	...
	<xsl:choose>
		<xsl:when test="/CharacterXMLDataset/Character/SpellCasterInfo/CasterClass[ClassName = $class]/MemorizedSpells  and $UseSpellsPrepared = 'Yes'">
			...
			<ul style="margin-top: 0px; margin-bottom: 0px; list-style-type: none"><li><b><xsl:value-of select="SlotLevel"/>
			<xsl:choose>
		...
		<xsl:otherwise>
			...
			<ul style="margin-top: 0px; margin-bottom: 0px; list-style-type: none"><li><b><xsl:value-of select="SpellLevel"/>
			<xsl:choose>
	...
	<xsl:if test="/CharacterXMLDataset/Character/ProhibitedSchools/ProhibitedSchool[ClassName = $class]">
		<b>Prohibited Schools: </b>
		<xsl:for-each select="/CharacterXMLDataset/Character/ProhibitedSchools/ProhibitedSchool[ClassName = $class]">
			<xsl:sort select="SchoolName"/>
...
<xsl:for-each select="Powers/ClassPowers[not(ClassName = preceding-sibling::ClassPowers/ClassName)]">
	<xsl:variable name="class" select="ClassName"/>
	<b><xsl:value-of select="ClassName"/> Powers Known</b>
	...
	<ul style="margin-top: 0px; margin-bottom: 0px; list-style-type: none"><li><b>Power Points </b> <xsl:value-of select="/CharacterXMLDataset/Character/ManifesterInfo/PowerPoints"/></li></ul>
	<xsl:for-each select="/CharacterXMLDataset/Character/Powers/ClassPowers[ClassName = $class]">
	<xsl:sort select="ClassName"/>
	<xsl:sort select="PowerLevel" order="descending"/>
	<xsl:variable name="level" select="PowerLevel"/>
	<ul style="margin-top: 0px; margin-bottom: 0px; list-style-type: none"><li><b><xsl:value-of select="PowerLevel"/>
	<xsl:choose>
...
<h1>Statistics</h1>
<b>Str </b> <xsl:value-of select="STR"/>, <b>Dex </b> <xsl:value-of select="DEX"/>, <b>Con </b> <xsl:value-of select="CON"/>, <b>Int </b> <xsl:value-of select="INT"/>, <b>Wis </b> <xsl:value-of select="WIS"/>, <b>Cha </b> <xsl:value-of select="CHA"/><br/>
...
<xsl:if test="Feats/Feat[Tags/text()[contains(., 'Special Action')]] != '' or Features/Feature[Tags/text()[contains(., 'Special Action')]] != ''">
	...
	<xsl:for-each select="Feats/Feat[Tags/text()[contains(., 'Special Action')]]">
		<b><a target="feat"><xsl:attribute name="href"><xsl:value-of select="HelpPage"/></xsl:attribute><xsl:value-of select="FeatName"/></a> </b>
		<xsl:value-of select="Description"/><br/>
	</xsl:for-each>
	<xsl:for-each select="Features/Feature[Tags/text()[contains(., 'Special Action')]]">
		<b><a target="feature"><xsl:attribute name="href"><xsl:value-of select="HelpPage"/></xsl:attribute><xsl:value-of select="FeatureName"/></a>
		<xsl:choose>
			<xsl:when test="FeatureType = 'Supernatural'"> (Su)</xsl:when>
			<xsl:when test="FeatureType = 'Spell-Like'"> (Sp)</xsl:when>
			<xsl:when test="FeatureType = 'Extraordinary'"> (Ex)</xsl:when>
		</xsl:choose><xsl:text> </xsl:text></b>
		<xsl:value-of select="Description"/><br/>
	</xsl:for-each>
</xsl:if>
